...
/**
 * This class provides an implementation of the DPH scoring function based on provided function. 
 * After mapping the input query terms and newsArticle info to DPHInNeed type, we calculate and store the 
 * dph score into a list of TextualDistanceInNeed, which is a class contains all information needed for 
...
public class DPHCalculator {
    ...
    public static Dataset<TextualDistanceInNeedList> calculateDPHScore(SparkSession spark, List<String> queryTerms, Dataset<NewsArticleInNeed> newsArticleInNeed) {
        ...
        // user queryTermsBroadcast to calculate both the TermFrequency (count of the term in the document) and the length of the document (in terms)
        ...
        // calculate the average document length in the corpus (in terms), the sum of term frequencies for the term 
